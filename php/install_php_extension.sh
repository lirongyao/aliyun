...
#zend
if ls -l /alidata/server/ |grep "5.3.29" > /dev/null;then
  ...
  echo "zend_loader.obfuscation_level_support=3" >> /alidata/server/php/etc/php.ini
  echo "zend_loader.license_path=" >> /alidata/server/php/etc/php.ini 
elif ls -l /alidata/server/ |grep "5.2.17" > /dev/null;then
  mkdir -p /alidata/server/php/lib/php/extensions/no-debug-non-zts-20090626/
  if [ $machine == "x86_64" ];then
...
#opensuse_memcache
if [ "$ifsuse" != "" ];then
	ln -s /alidata/server/php/lib64/extensions/no-debug-non-zts-20100525/memcache.so  /alidata/server/php/lib/php/extensions/no-debug-non-zts-20100525/memcache.so
	ln -s /alidata/server/php/lib64/extensions/no-debug-non-zts-20121212/*  /alidata/server/php/lib/php/extensions/no-debug-non-zts-20121212/
fi
